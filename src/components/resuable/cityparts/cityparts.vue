	...
	<div class="areaparts">
		<table>
			<tr>
				<td>省份</td>
				<td>
				<Select v-model="prov" style="width:210px">
				<Option v-for="op in arr" :value="op.name" :key="op.name">
					{{ op.name }}
				</Option>
				</Select>
				</td>
			...
			<tr>
				<td>城市</td>
				<td>
				<Select v-model="city" style="width:210px">
					<Option v-for="op in cityArr" :value="op.name" :key="op.name">
						{{ op.name }}
					</Option>
				</Select>
				</td>
			...
			<tr>
				<td>区</td>
				<td>
			    <Select v-model="district" v-if="district" style="width:210px">
			    	<Option v-for="op in districtArr" :value="op.name" :key="op.name">
			    		{{ op.name }}
			    	</Option>
			    </Select>
				</td>
			...
			<tr>
				<td>具体地址</td>
				<td>
			    <Input v-model="value2" @on-change="getApparts" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :rows="4" placeholder="请输入详细地址" style="width:400px" v-validate="'required|max:200|min:0'" name="具体地址"></Input>
			    <span v-show="errors.has('具体地址')"  style="color:red;">{{ errors.first('具体地址') }}</span>
				</td>
			</tr>
		</table>
	</div>
...
export default {
	...
	methods: {
		updateCity: function () {
			...
			this.$store.state.cityparts = []
			this.$store.state.cityparts.push(this.prov, this.city, this.district)
		},
		...
		getApparts () {
			this.$store.state.cityaddress = ''
			this.$store.state.cityaddress = this.value2
		}
	},
...
.areaparts table td:first-child{
	text-align:right;
	min-width:100px;
	word-wrap:normal;
}
.areaparts table td:last-child{
	text-align:left;
	padding-left:20px;
}
</style>
